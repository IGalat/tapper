...
On `tapper.start`, the previous dictionary gets transformed into `Tap`s, and text action into `send(text)`.
...
Now we specified the conditions when this Tap can trigger: only when `notepad` is the foreground window (title or exec name),
...
Now `print screen` will press `escape` instead.

Wait, why is `esc` in a weird `$()` brackets? Well, otherwise letters e,s,c would be sent -
any non single-character key should be inside `$()` for the send command.
You can change the brackets in `tapper.config`, as well as default interval for `send`.
...
To recap, add Groups, Taps or dicts to root Group, and then call `tapper.start()` to launch!
...
- Controllers: allow you to check status and give commands. ```from tapper import kb, mouse, window```
  - [Keyboard](https://github.com/IGalat/tapper/blob/master/src/tapper/controller/keyboard/kb_api.py).
  Get key state or issue commands. For commands `send` is more convenient though.
  - [Mouse](https://github.com/IGalat/tapper/blob/master/src/tapper/controller/mouse/mouse_api.py).
  Same thing.
  - [Window](https://github.com/IGalat/tapper/blob/master/src/tapper/controller/window/window_api.py).
...
- Events pub/sub: subscribe or unsubscribe from keyboard/mouse events to get all non-emulated signals.
...
If bool(callable) is True for every `Condition` for that element, it can trigger.

Here are existing args:
...
- click other keyboard/mouse keys, with aliases: `$(ctrl;lmb)` (same as `$(ctrl)$(lmb)`) will press left control, then left mouse button.
- press combinations: `$(alt+ctrl+c,v,v)` will press alt, ctrl, then click c, v, v, and release alt and control.
- sleep: `$(50ms;3s)` will sleep for 50 milliseconds(0.05 sec), and then 3 sec.
...
Warning: "b" will stay pressed after this, which when unintended can cause bad experience. Press "b" manually to release.
...
and [mouse](https://github.com/IGalat/tapper/blob/master/src/tapper/model/keyboard.py).
...
```python
Tap("t", repeat_while(lambda: datetime.datetime.now().hour >= 22, send("Go to sleep!"), 1200, 4))
```

This will upon pressing `t` start looping, and if the hour is 22 or later,
and will type the phrase every immediately and then every 20 min (1200 sec).
...
Tap("b", repeat_while_pressed("ctrl", retry_my_stuff_function))
```

Pressing `a` will do at most 15 iterations, 1 second apart, and pressing `b` will use defaults - infinite retries 0.1 sec apart.
...
```python
Tap("ctrl+l", toggle_repeat(send("\nLFG ICC 25HM rsham 6.2gs\n"), 27))
```

One `ctrl+l` click will start sending the phrase every 27 sec, another will stop it.
