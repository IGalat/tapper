...
- keylog helper, allows to see what keys were pressed since its start()/wait for key. has filter of keys, not just all
...
- add "key was pressed, since now/since X seconds from now". This would allow running something until key was pressed,
  without helper.toggle
